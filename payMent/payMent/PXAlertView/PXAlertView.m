...
- (id)initWithTitle:(NSString *)title
            messageWithAtrribute:(NSAttributedString *)message
        cancelTitle:(NSString *)cancelTitle
         otherTitle:(NSString *)otherTitle
 buttonsShouldStack:(BOOL)shouldstack
        contentView:(UIView *)contentView
         ...
         completion:(PXAlertViewCompletionBlock)completion
{
    ...
    if (self) {
        ...
        self.messageLabel.backgroundColor = [UIColor clearColor];
        self.messageLabel.textColor = [UIColor whiteColor];
        if (message_alignment_left == 1) {
            //set once, work once,
            message_alignment_left = 0;
...
-(NSMutableArray*)boldFontArray
{
    if (!_boldFontArray) {
        _boldFontArray = [NSMutableArray new];
        [_boldFontArray addObject:NSLocalizedString(@"I Sold it", nil)];
        
    }
    
    return _boldFontArray;
}
...
+ (BOOL)isCurrentViewShowed{
    return [[PXAlertViewStack sharedInstance].alertViews count] > 0;
}
...
-(void)setButtonTitleBold:(NSString*)title
{
...
///////////////////////
+ (instancetype)showAlertWithTitle:(NSString *)title
              messageWithAttribute:(NSAttributedString *)message
                       cancelTitle:(NSString *)cancelTitle
                        otherTitle:(NSString *)otherTitle
                        completion:(PXAlertViewCompletionBlock)completion
{
...
static int message_alignment_left = 0;
+(void)setMessageAlignmentLeft
{
    message_alignment_left = 1;
}
+(void)setMessageAlignmentCenter
{
    message_alignment_left = 0;
}
